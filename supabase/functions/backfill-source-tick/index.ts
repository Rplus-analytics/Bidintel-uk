...
let PCS_HTTP_CLIENT: any = null;
function getPcsClient() {
  if (PCS_HTTP_CLIENT !== null) return PCS_HTTP_CLIENT;
  try {
    // @ts-ignore Deno.createHttpClient is available in the edge runtime.
    PCS_HTTP_CLIENT = (Deno as any).createHttpClient?.({
      caCerts: [SECTIGO_INTERMEDIATE_PEM],
    }) ?? undefined;
  } catch (_) { PCS_HTTP_CLIENT = undefined; }
  return PCS_HTTP_CLIENT;
}
  ...
  if (client) {
    try {
      const res = await fetch(url, {
        // @ts-ignore client option is supported by the Deno fetch.
        client,
