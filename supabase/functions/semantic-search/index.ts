...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("search_tenders_hybrid", {
    ...
    if (error) {
      ...
      return new Response(
        JSON.stringify({ results: [], embeddingAvailable: queryEmbedding !== null, embedError, rpcError: msg, expansion, count: 0 }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }
