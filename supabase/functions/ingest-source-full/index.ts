...
// ?source=fts|ted|pcs (default fts) — ingests last 24h into tenders_<source>.
...
Deno.serve(async (req) => {
  ...
  if (auth !== Deno.env.get("INGEST_SECRET")) return new Response("Unauthorized", { status: 401 });

  const url = new URL(req.url);
  const source = (url.searchParams.get("source") ?? "fts").toLowerCase();
  ...
  else return new Response(JSON.stringify({ error: "source must be fts|ted|pcs" }), { status: 400 });

  return Response.json({ source, ...result, duration_ms: Date.now() - start });
});
