...
CREATE OR REPLACE FUNCTION public.search_tenders_hybrid(
 ...
 RETURNS TABLE(
   ...
   hybrid_score double precision, matched_terms text[], matched_cpvs text[]
 )
 LANGUAGE plpgsql
 STABLE
...
BEGIN
  ...
  final AS (
    SELECT e.*,
      ...
      -- penalty: only generic context, no core, no CPV
  ...
  WHERE (f.s_kw + f.s_cpv + f.s_sem + f.bonus) > 0
    -- drop pure-context noise entirely when we have any core/CPV signal across pool
