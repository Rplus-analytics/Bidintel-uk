...
DROP FUNCTION IF EXISTS public.search_tenders_hybrid(
  vector, text, integer, timestamptz, text, text[], text[],
...
BEGIN
  ...
  WITH sem AS (
    ...
    WHERE query_embedding IS NOT NULL AND t.embedding IS NOT NULL AND t.published_at >= since_ts
      AND (NOT active_only OR t.derived_status IS NULL OR t.derived_status IN ('active','planned'))
  ...
  scored AS (
    SELECT
      ...
      t.derived_status AS s_status, t.deadline_at AS s_deadline,
