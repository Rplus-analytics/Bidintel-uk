...
BEGIN
  ...
  ranked AS (
    SELECT g.*,
      ...
      CASE
        ...
        ELSE 4
      END AS src_rank,
      lower(regexp_replace(coalesce(g.s_title,''), '\s+', ' ', 'g'))
        || '|' || lower(coalesce(g.s_buyer,'')) AS dedup_key
    FROM gated g
  ...
  deduped AS (
    ...
    FROM ranked r
    ORDER BY r.dedup_key, r.hybrid DESC, r.src_rank ASC, r.s_pub DESC NULLS LAST
  )
  SELECT
