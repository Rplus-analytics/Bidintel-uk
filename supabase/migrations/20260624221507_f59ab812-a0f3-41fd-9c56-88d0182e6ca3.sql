...
LANGUAGE plpgsql STABLE SET search_path TO 'public' SET statement_timeout TO '30s'
AS $function$
DECLARE
...
BEGIN
    ...
    SELECT e.*,
      (CASE WHEN e.s_cpv >= 1.0 THEN 0.5
       ...
       + CASE e.s_source
           WHEN 'cf' THEN 0.20
           WHEN 'fts' THEN 0.20
           WHEN 'contracts_scotland' THEN 0.10
           WHEN 'ted' THEN 0.05
           ELSE 0
         END
       + CASE
  ...
  SELECT
    ...
    f.s_sem, f.s_kw, f.s_cpv, f.bonus, f.penalty,
    (f.s_kw * w_keyword + f.s_cpv * w_cpv + f.s_sem * w_semantic + f.bonus - f.penalty)::float AS hybrid,
    f.s_mterms, f.s_mcpvs
  FROM final f
  WHERE (f.s_kw + f.s_cpv + f.s_sem) > 0
  ORDER BY (f.s_kw * w_keyword + f.s_cpv * w_cpv + f.s_sem * w_semantic + f.bonus - f.penalty) DESC
  LIMIT match_count;
END;
$function$;