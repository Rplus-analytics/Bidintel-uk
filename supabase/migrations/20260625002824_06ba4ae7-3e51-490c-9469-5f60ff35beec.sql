CREATE OR REPLACE FUNCTION public.search_tenders_hybrid(
  ...
  intent_domain text DEFAULT NULL::text
)
RETURNS TABLE(
  ...
  intent_penalty double precision, intent_mismatch boolean
)
LANGUAGE plpgsql
STABLE
...
DECLARE
  ...
  ctx_lc text[];
  expanded_query text;
  -- Physical-service domains that should never be dominated by software/IT rows.
  physical_domains text[] := ARRAY['catering','cleaning','fm','waste','grounds','care','nursing','transport','taxi','construction','training'];
  is_physical_intent boolean := intent_domain IS NOT NULL AND intent_domain = ANY(physical_domains);
BEGIN
  all_terms := ARRAY(
  ...
  scored AS (
    SELECT
      ...
      -- Is this row a software/IT procurement? (primary CPV in 72xx or 48xx family)
      (coalesce(t.primary_cpv,'') LIKE '72%' OR coalesce(t.primary_cpv,'') LIKE '48%') AS s_is_it
  ...
  final AS (
    SELECT e.*,
      ...
      -- Procurement-intent guard: physical-service intent + IT/software row + no matching IT CPV in this row.
      (CASE
         WHEN is_physical_intent
              AND e.s_is_it
              AND e.s_cpv < 0.7  -- row didn't match any of the *allowed* domain CPVs strongly
         THEN 2.00
         ELSE 0
       END)::float AS intent_pen,
      (is_physical_intent AND e.s_is_it AND e.s_cpv < 0.7) AS intent_mis,
      (CASE
  ...
  ranked AS (
    SELECT g.*,
      ...
      (g.penalty_base + g.low_conf_penalty + g.intent_pen)::float AS penalty,
      (g.s_kw * w_keyword + g.s_cpv * w_cpv + g.s_sem * w_semantic
        + g.src_bonus + g.st_bonus + g.rel_bonus
        - (g.penalty_base + g.low_conf_penalty + g.intent_pen)
      )::float AS hybrid,
      CASE
