...
SELECT 'cf_native'::text AS source,
  ...
  COALESCE((j.p->>'awardedDate')::timestamptz,
           CASE WHEN ((j.brel->'awards'->0)->>'date') ~ '^[12][0-9]{3}-'
                THEN ((j.brel->'awards'->0)->>'date')::timestamptz END) AS awarded_date,
...
SELECT 'cf_ocds'::text AS source,
  ...
  COALESCE(
    CASE WHEN (o.award->>'date') ~ '^[12][0-9]{3}-' THEN (o.award->>'date')::timestamptz END,
    CASE WHEN ((o.brel->'awards'->0)->>'date') ~ '^[12][0-9]{3}-' THEN ((o.brel->'awards'->0)->>'date')::timestamptz END
  ) AS awarded_date,
