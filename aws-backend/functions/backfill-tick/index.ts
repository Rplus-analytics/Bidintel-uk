// Multi-source historical backfill tick. Walks every non-completed, non-"_full"
// row in backfill_state one month at a time, across FTS / CF / TED / Sell2Wales /
// eTenders Ireland.
//
// TRIGGER: CRON -> EventBridge. Gated on INGEST_SECRET in the original because
// config.toml sets verify_jwt = false, making it a public URL.
//
// Ported from supabase/functions/backfill-tick (Deno). Every fetcher
// (backfillFts, backfillCf, backfillTed, backfillOcdsFeed, backfillEtendersIreland,
// backfillEtendersNi), upsertRows with its TED-vs-OCDS branch, parseTedDate,
// monthRange, nextMonth, isCurrentOrFuture and runOne are byte-identical.
//
// NOTE the deliberately-disabled CPV filter: matchesCpv() unconditionally returns
// true here, with a comment explaining that 2010-2014 notices lack consistent CPV
// codes. TARGET_CPV_PREFIXES is consequently DEAD CODE in this file — it is
// declared and never read. Preserved as-is.
//
// TIMEOUT: no wall-clock budget at all — only MAX_ROWS_PER_TICK (200) per source
// and hard page caps (200 pages, startAt > 20000). All sources run CONCURRENTLY
// via Promise.all, so one slow source does not block the others, but nothing stops
// the whole tick running long. Give it 900s.
...
async function run(detail: Record<string, any>): Promise<any> {
  // INGEST_SECRET check dropped: EventBridge invocation is IAM-authorised, not a
  // public URL. Re-add only if this is also exposed through API Gateway.
  ...
  if (error) {
    throw new Error(`backfill_state read: ${error.message}`);
  }
...
export const handler = async (event: ScheduledEvent | { detail?: Record<string, any> }): Promise<any> => {
  ...
  if (!isDbConfigured()) {
    const message =
      "backfill-tick is scaffolded but not wired to RDS. See functions/_shared/db.ts TODO(rds). " +
...
// Exported for offline verification of the cursor arithmetic.
export { monthRange, nextMonth, isCurrentOrFuture, parseTedDate, matchesCpv };
