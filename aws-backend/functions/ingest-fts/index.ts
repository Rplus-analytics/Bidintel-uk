// Daily Find a Tender (FTS) OCDS ingest into tenders (CPV-filtered).
//
// Ported from supabase/functions/ingest-fts (Deno) as an EventBridge-invoked Lambda.
// Every helper (extractCpv, matchesCpv, pickRegion, mapStatus), the CPV prefix whitelist, the
...
async function run(detail: Record<string, any>, ctx?: any): Promise<any> {
  ...
  if (win && !blocksWatermark(errors)) {
    try {
      await commitWindow(supabase, "fts", win);
    } catch (e: any) {
      errors.push({ watermark: e?.message ?? String(e) });
    }
  } else {
    console.warn(JSON.stringify({
      source: "fts", watermark: "NOT advanced", reason: "run reported errors that indicate the window was not retrieved",
...
export const handler = async (event: ScheduledEvent | { detail?: Record<string, any> }, context?: any): Promise<any> => {
  ...
  if (!isDbConfigured()) {
    const message =
      "ingest-fts has no database configuration. Set DATABASE_SECRET_ARN.";
